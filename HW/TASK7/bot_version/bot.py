...
def reply(message):
        ...
        dict_return = centrality_and_params_graph(preparing_graph
                                                  (model_start(input_words)))
        showing_graph(preparing_graph_for_show(model_start(input_words)),
                      model_start(input_words),
                      communities1(preparing_graph(model_start(input_words))),
                      degree_counter1(preparing_graph
                                      (model_start(input_words))))
        text = str(dict_return)
        user = message.chat.id
        bot.send_message(user, text)
        bot.send_photo(user, 'foo.png')
